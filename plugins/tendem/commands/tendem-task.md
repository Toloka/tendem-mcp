...
1. Call `create_task` **now** — `description` is the user's instruction above,
...
2. If the instruction references local files, upload them immediately after
   creating the task (`get_file_upload_url` + the skill's
   `scripts/tendem-upload.sh`, then the naming `send_message`).
